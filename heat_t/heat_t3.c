...
#define BODY_X_START 4
#define BODY_X_END 6
#define BODY_Y_START 4
#define BODY_Y_END 6
...
__global__ void initialize_kernel(double *h, int n) {
    ...
    int body_x_start = (BODY_X_START * n) / ROOM_SIZE;
    int body_x_end = (BODY_X_END * n) / ROOM_SIZE;
    int body_y_start = (BODY_Y_START * n) / ROOM_SIZE;
    int body_y_end = (BODY_Y_END * n) / ROOM_SIZE;

    if (i < n && j < n) {
        if (i >= body_x_start && i <= body_x_end && j >= body_y_start && j <= body_y_end) {
            h[i * n + j] = BODY_TEMP;
